...
.version{font-size:13px;color:#555}
/* Tab content */
...
<section id="tab-history" class="tab-content" hidden>
  ...
  <div id="history-table-wrap" hidden>
    <table id="history-table">
      <thead><tr>
        <th>Thời gian</th><th>SOC</th><th>Qty</th><th>Note</th><th></th>
      </tr></thead>
...
<section id="tab-settings" class="tab-content" hidden>
  ...
  <div class="settings-section">
    <h3>ℹ️ Phiên bản</h3>
    <p style="color:#ccc">SOC Scanner v1.0.0</p>
    <p class="hint">Cache version: soc-v1.0.0</p>
  </div>
  <div class="settings-section">
...
function _validateSoc(text) {
  const s = text.trim().replace(/[\r\n\t\s]/g, '');
  return SOC_REGEX.test(s) ? s : null;
}
...
function pauseScanner() { if (_qr && _qrRunning) try { _qr.pause(true); } catch(e) {} }
function resumeScanner() { if (_qr && _qrRunning) try { _qr.resume(); } catch(e) {} }

function _onScan(text) {
  const soc = _validateSoc(text);
  ...
  pauseScanner(); beepSuccess(); vibrate(200); flashScreen('#00ff88', 150);
  st.textContent = '✓ ' + soc;
  document.dispatchEvent(new CustomEvent('soc-scanned', { detail: { soc } }));
}
...
function _openSht(soc) {
  _curSoc = soc; _socDisp.textContent = soc;
...
function _closeSht() { _sht.hidden = true; _bkd.hidden = true; _curSoc = null; }
...
_frmQty.addEventListener('submit', async e => {
  ...
  try {
    await addRecord({ soc: _curSoc, quantity: qty, note: _noteIn.value || null });
    toast('Đ\xe3 lưu ' + _curSoc + ' \xd7 ' + qty, 'success');
    _closeSht(); resumeScanner();
...
document.getElementById('btn-cancel').addEventListener('click', () => { _closeSht(); resumeScanner(); });
_bkd.addEventListener('click', () => { _closeSht(); resumeScanner(); });
document.addEventListener('soc-scanned', e => _openSht(e.detail.soc));
...
async function refreshHistory(q) {
  try {
    ...
    const tbody=document.getElementById('history-tbody'), empty=document.getElementById('history-empty'), wrap=document.getElementById('history-table-wrap');
    if (!recs.length) { empty.hidden=false; wrap.hidden=true; }
    else {
      empty.hidden=true; wrap.hidden=false;
      tbody.innerHTML = recs.map(r =>
        '<tr><td class="col-time">'+_fmtTime(r.timestamp)+'</td>'+
        '<td class="col-soc">'+_esc(r.soc)+'</td>'+
        '<td class="col-qty">'+_esc(String(r.quantity))+'</td>'+
        '<td class="col-note" title="'+_esc(r.note??'')+'">'+_esc(_trunc(r.note))+'</td>'+
        '<td><button class="btn-delete-row" data-id="'+_esc(String(r.id))+'" aria-label="X\xf3a">🗑️</button></td></tr>'
      ).join('');
    }
...
async function exportCurrentRecords() {
  try {
    ...
    const BOM = '﻿', HDR = 'time,SOC,Qty,Note\r\n';
    const rows = recs.map(r => [_fmtTime(r.timestamp),r.soc,r.quantity,r.note??''].map(_csvFld).join(',')).join('\r\n') + '\r\n';
