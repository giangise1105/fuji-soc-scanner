...
.col-date{font-size:12px;color:#888;white-space:nowrap}
@media(max-width:359px){.col-date{display:none}}
...
<section id="tab-history" class="tab-content" hidden>
  ...
  <div id="history-table-wrap" hidden>
    <table id="history-table">
      <thead><tr>
        <th>Thời gian</th><th>SOC</th><th class="col-date">Ngày SX</th><th>Qty</th><th>Note</th><th></th>
      </tr></thead>
  ...
  try {
    ...
    if (!recs.length) { empty.hidden=false; wrap.hidden=true; }
      ...
      tbody.innerHTML = recs.map(r =>
        ...
        '<td class="col-date">'+_esc(r.productionDate||'—')+'</td>'+
